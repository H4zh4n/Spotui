...
object TidalAudioProvider {
        ...
        for (track in tracks.take(streamCandidateLimit)) {
            for (quality in streamQualityCandidates(track, preferAtmos, audioQuality)) {
                ...
                val streamAttempt = runCatching {
                    ...
                    Log.w("TidalAudio", "TIDAL $quality stream failed for ${track.trackId}", error)
    ...
    private fun findCandidateTracks(
        ...
        resolveSongLinkTidalTrackId(query)?.let { tidalId ->
            ...
            if (score >= MIN_MATCH_SCORE) {
                ...
                Log.w("TidalAudio", "Deferred lossy song.link TIDAL match $tidalId for ${query.title}: score=$score")
            } else {
                Log.w("TidalAudio", "Ignored weak song.link TIDAL match $tidalId for ${query.title}: score=$score")
            }
        }
        return candidates
    ...
    private fun requestDirectFlac(
        ...
        for (endpoint in endpoints) {
            ...
            Log.w("TidalAudio", "TIDAL resolver ${endpoint.name} failed for ${track.trackId}", error)
        }
    ...
    private fun resolveSongLinkTidalTrackId(query: Query): String? {
        for (sourceUrl in songLinkSourceUrls(query.mediaId)) {
            ...
            runCatching {
                ...
                Log.i("TidalAudio", "Resolved TIDAL track $trackId through song.link from $sourceUrl")
                return trackId
            }
        }
        return null
